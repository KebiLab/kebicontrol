...
pub async fn apply(op: InputOp, text: Option<&str>) -> Result<Option<String>> {
    ...
    let msg = match op {
        ...
        InputOp::Click => { enigo.button(enigo::Button::Left, Click).ok(); "Клик" }
        InputOp::RightClick => { enigo.button(enigo::Button::Right, Click).ok(); "Правый клик" }
        InputOp::DoubleClick => {
        ...
        InputOp::Scroll => {
            let amount: i32 = text.and_then(|s| s.parse().ok()).unwrap_or(3);
